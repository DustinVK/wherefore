---
description: Inspect the codebase and propose starter areas + topics for the discussion log
argument-hint: [optional focus path]
allowed-tools: Read, Grep, Glob, Bash(ls:*), Bash(find:*), Bash(cat:*), Bash(git:*), Write
---

You are bootstrapping the controlled vocabulary for this project's discussion log —
the two-facet `discussions/topics.md` used by the `discussion-log` and
`discussion-read` skills. Read the codebase, propose a draft vocabulary, and — only
after the user confirms — write it.
...
- If `discussions/topics.md` does NOT exist: create the `discussions/` directory
  (plus an empty `INDEX.md` if missing) and write the confirmed vocabulary as
  `discussions/topics.md` in the two-section format above.
